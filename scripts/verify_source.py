...
def main() -> None:
    ...
    main_activity_text = (root / "project/app/src/main/java/com/greenbuddy/acevosetupengineer/MainActivity.java").read_text(encoding="utf-8")
    require("new RacePlacePackageProvider()" in main_activity_text
            and "new SetupsMarketProvider()" in main_activity_text,
            "both LIVE providers must remain enabled")
    require("Intent.ACTION_CREATE_DOCUMENT" in main_activity_text,
            "Android document export is missing")
    require('intent.setType("application/octet-stream")' in main_activity_text,
            "binary .carsetup MIME contract is missing")
    require(' + ".carsetup"' in main_activity_text,
            ".carsetup filename contract is missing")
    require("output.write(pendingExportBytes)" in main_activity_text,
            "binary .carsetup bytes are not written")
    require(".txt" not in main_activity_text,
            "text-only setup export regression detected")
    require("Color.rgb(255, 213, 74)" in main_activity_text,
            "yellow app accent is missing")
    require("Color.rgb(215, 25, 32)" not in main_activity_text,
            "obsolete red app accent remains")

    build_text = (root / "project/app/build.gradle").read_text(encoding="utf-8")
    require("versionCode 5" in build_text and "versionName '1.1.2'" in build_text,
            "Android release version is not 1.1.2")

    style_text = (root / "project/app/src/main/res/values/styles.xml").read_text(encoding="utf-8")
    require("#FFD54A" in style_text and "#D71920" not in style_text,
            "yellow Android theme contract failed")
    ...
    report = {
        ...
        "liveProviders": ["RacePlace", "SetupsMarket"],
        "exportFormat": ".carsetup binary",
