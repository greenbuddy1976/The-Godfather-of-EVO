...
- three complete LIVE rounds across every configured provider;
...
- Searches RacePlace and SetupsMarket in exactly three complete rounds. A
...
- If no exact setup exists, it can use a user-selected same-car `.carsetup` as
  a structure carrier. Carrier numbers are discarded; SELF CALC rewrites all
  verified adjustable fields from the car ranges and exact-layout model and
  labels the output `ENGINEERING MODEL`.
